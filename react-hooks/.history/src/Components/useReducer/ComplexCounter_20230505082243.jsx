...
export default function ComplexCounter() {
  ...
  return (
    <Fragment>
      ...
      <Button data={() => dispatch({ type: increament, value: 1 })} />
      <Button
        data={() => dispatch({ type: increament, value: 1 })}
        text={" decreament by 1"}
      />

      <button onClick={() => dispatch({ type: decreament, value: 1 })}></button>

      <h2>Count: {state.count2}</h2>
      <button onClick={() => dispatch({ type: increament2, value: 2 })}>
        increament by 2
      </button>
      <button onClick={() => dispatch({ type: decreament2, value: 2 })}>
        decreament by 2
      </button>
    </Fragment>
  );
