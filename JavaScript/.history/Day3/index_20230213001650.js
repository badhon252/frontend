...
const getDeveloperProjects = (developersId) => {
    const developer = developers.find(developer => developer.id === developersId);
    const developerProjects = developer.projects;
    const projectNames = projects.filter(project => developerProjects.includes(project.id)).map(project => project.name);
    return projectNames;
    }
