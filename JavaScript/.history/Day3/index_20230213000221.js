...
function getDeveloperPorjects(developers, projects){
    ...
    const d1 = developers.map((dev, i)=>{
        return `${dev.name} ${dev.projects[i] == projects.map(pr=>pr.id) ? 'is working on' : 'is not working on'} ${projects.map(pr=>pr.name)}`
    ...
    // let devName = developers.map((pId)=>(pId.name))
    // console.log(devName)
