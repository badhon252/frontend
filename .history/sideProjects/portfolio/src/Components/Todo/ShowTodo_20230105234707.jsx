...
export default function ShowTodo({todo,todos}) {
            ...
            {todo.map(item=>{
                return ( 
                 <div className={Classes.list} key={item.id}>
                    <input className={Classes.checkbox} type="checkbox" onClick={handleCheck} htmlFor="checkbox" ></input> 
                    <li className={Classes.li} id="checkbox" >{item.text}</li>
                 </div>
                   )}
            )}
